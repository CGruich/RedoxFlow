...
class RedoxPotentialScript(NWChemInputConstructor):
    def __init__(
        ...
        self.batch = False
        # If any of the inputted values is a list, then we are generating input scripts in batches.
        for key, value in self.inputs.items():
            if type(value) is list:
                # Activate batch mode
                self.batch = True
            ...
            # Otherwise, we are only generating one input script.
            # Activate single-script generation
            script = self.construct_input_script()
